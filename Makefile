...
REGISTRY ?= quay.io
REPOSITORY ?= $(REGISTRY)/redhat-cop/keepalived-operator
...
export GITHUB_PAGES_REPO ?= redhat-cop/keepalived-operator
export HELM_CHARTS_SOURCE ?= charts
export HELM_CHART_DEST ?= $(GITHUB_PAGES_DIR)

LDFLAGS := "-X github.com/redhat-cop/keepalived-operator/version.Version=$(VERSION) \
	-X github.com/redhat-cop/keepalived-operator/version.Vcs=$(BUILD_COMMIT) \
	-X github.com/redhat-cop/keepalived-operator/version.Timestamp=$(BUILD_TIMESTAMP) \
	-X github.com/redhat-cop/keepalived-operator/version.Hostname=$(BUILD_HOSTNAME)"
...
manager: generate fmt vet
	go build -o build/_output/bin/keepalived-operator  -ldflags $(LDFLAGS) github.com/redhat-cop/keepalived-operator/cmd/manager

# Build manager binary
manager-osx: generate fmt vet
	GOOS=darwin go build -o build/_output/bin/keepalived-operator -ldflags $(LDFLAGS) github.com/redhat-cop/keepalived-operator/cmd/manager
...
docker-build:
	...
	go get -u github.com/rjeczalik/notify@v0.9.2
